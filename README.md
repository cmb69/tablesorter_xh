...
3.  Upload the whole folder `tablesorter/` to your server into the `plugins/`
    folder of CMSimple_XH.
4.  Set write permissions for the subdirectories `config/`, `css/` and
    `languages/`.
5.  Go to `Plugins` → `Tablesorter` in the back-end to check if all
    requirements are fulfilled.

## Settings

The configuration of the plugin is done as with many other CMSimple_XH
plugins in the back-end of the website. Go to `Plugins` → `Tablesorter`.
...
If you also assign the CSS class `tablesorter_columns` to the table,
users can select which columns are shown.

The sorting of the rows works by case-insensitive string comparison.
This does not work well for numeric
columns, so it is possible to mark a numeric column as such by adding
the CSS class `tablesorter_numeric` to the `<th>` of the column. Note that
thousands separators are not supported, and that only dots (`.`) are
supported as decimal separator.  An alternative is to add leading zeros to
numbers, and to format them in a way, that alphanumeric ordering gives the desired
results (and not to use `tablesorter_numeric`).  Sorting arbitrary dates and/or times is
...
along with Tablesorter_XH.  If not, see <https://www.gnu.org/licenses/>.
...
Mooon](https://code.google.com/u/newmooon/). Many thanks for publishing
this icon under GPL.

Many thanks to the community at the [CMSimple_XH
forum](https://www.cmsimpleforum.com) for tips, suggestions and testing.
