...
// jshint browser:true,esversion:5,strict:implied
// @ts-check
...
function setClass(element, className) {
    var newClassName = element.className.replace(
        /(?:^|\s)tablesorter_(?:asc|desc|ascdesc)(?!\S)/,
        ""
    );
    if (newClassName !== "") {
        newClassName += " ";
...
function Widget(table) {
    ...
    function sort(column, desc, numeric) {
        ...
        Array.from(rows).forEach(function (value) {
    ...
    function determineHiddenColumns() {
        ...
        for (var prop in Widget.breakpoints) {
            if (
                Widget.breakpoints.hasOwnProperty(prop) &&
                window.innerWidth < Widget.breakpoints[prop]
            ) {
        ...
        if (hiddenColumns.length) {
            table.querySelectorAll("tr").forEach(function (row) {
                ...
                if (row.parentNode.nodeName.toLowerCase() === "tbody") {
                    var section = /** @type {HTMLTableSectionElement} */ (row.parentNode);
                    var button = document.createElement("button");
                    button.className = "tablesorter_expand";
                    button.textContent = TABLESORTER.show;
                    button.onclick = function () {
